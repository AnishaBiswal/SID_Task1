...
if (isset($_POST['name']) && isset($_POST['uname'])
	...
	&& isset($_POST['domain']) && isset($_POST['password'])
...

{
	...
	if (empty($uname)) {
        ...
        header("Location: signup.php?error=branch is required&$user_data");
	    exit();
	}
	else if(empty($year)){
        ...
        header("Location: signup.php?error=domain is required&$user_data");
	    exit();
	...
	if($pass !== $re_pass){
		...
		if (mysqli_num_rows($result) > 0) {
           ...
           $sql2 = "INSERT INTO users(user_name, password, name, email, branch, year, domain, contact) VALUES('$uname', '$pass', '$name', '$email', '$year', '$contact', '$branch' )";
